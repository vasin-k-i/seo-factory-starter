...
делал это не «как получится», в репозитории лежит скилл — ставится одной командой
и работает потом в любой папке:
...
**Что стоит доставить отдельно.** Эти скиллы в комплект не входят (чужие, со своими
лицензиями) — ищите по названию в маркетплейсах Claude Code:

- **Разведка ниши** — go/no-go по нише до того, как строить сайт: спрос, конкуренты,
  деньги-страницы, контент-план. Публичный репозиторий:
  [vasin-k-i/razvedka-nishi](https://github.com/vasin-k-i/razvedka-nishi).
- **Дизайн и фронтенд:** `impeccable`, `ui-ux-pro-max`, `frontend-design`,
  `high-end-visual-design`, `design-taste-frontend`, `redesign-existing-projects`,
  `image-to-code`, `apple-design`.
- **Парсинг:** `scrapling-official` — когда сайт конкурента не читается обычным запросом
  (403, антибот, JS-рендер).
